...
export default function CategoryDetails() {
    ...
    let { addToCard, setCountCard, addToWithList } = useContext(CardContext)
    useEffect(() => {
        async function getRelatedData() {
            try {
                const { data } = await axios.get('https://ecommerce.routemisr.com/api/v1/products');
                const filteredProducts = data.data.filter((product) => product.category._id === id);
    ...
    return (
        <div className="my-8">
            <div className="row gap-4 justify-start ">
